...
loopres <- foreach(a = 1:Nmaxa) %:%
			...
			set.seed(8888*as.numeric(paste(s,a,a,a, sep = "")))
...
#			}
			...
			if(sum(which(optgrad>=10e-6))==0) {
				...
				parsRE[3:4] <- estpars[3:4] - truepars[3:4]
				...
				save(replist, file = file.path(getwd(), dir.new, paste0("a", a, "s", s,".RData")))
				return(replist)
			}
		}
#	}
...
loopres <- foreach(a = 1:Nmaxa) %:%
			foreach(s = 1:Nsims) %dopar% {
				repfile <- file.path(getwd(), dir.new, paste0("a", a, "s", s,".RData"))
...
source(file.path(dir.check, "Results_NoTVG.R"))
