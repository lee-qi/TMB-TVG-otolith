setwd("G:/My Drive/TMB_TVG")
setwd("/Users/LeeQi/Google Drive/TMB_TVG")

dir.check <- getwd()
dir.new <- "Change sigInc"
...
#sigIncvec <- seq(from = 0.001, to = 0.005, by = 0.001)
#sigIncvec <- seq(from = 0.0001, to = 0.0015, by = 0.0002)
sigIncvec <- c(0.0001, 0.001, 0.01, 0.1, 0.5)
...
#Nfish <- 500
#minage <- 0.7
Nfishvec <- c(20, 20, 100)
minagevec <- c(0.1, 0.7, 0.7)
...
#foreach(a = 1:Nmaxa) %:%
	...
	sigInc <- sigIncvec[v]
	set.seed(8888*as.numeric(paste(s,v,a, sep = "")))
	...
	setwd(dir.check)
#	compile("TVG_Windows.cpp")
	dyn.load(dynlib("TVG_Windows"))
	
	objfun <- MakeADFun(dat, pars, DLL = "TVG_Windows", hessian = TRUE, silent = TRUE,
	...
	if(sum(which(optgrad>=10e-6))==0) {
		...
		save(replist, file = file.path(getwd(), dir.new, paste0("a", a, "v", v, "s", s,".RData")))
		return(replist)
	}
}
#	}
...
loopres <- foreach(a = 1:Nmaxa) %:%
		foreach(v = 1:Nvar) %:%
			foreach(s = 1:Nsims) %dopar% {
				repfile <- file.path(dir.check, dir.new, paste0("a", a, "v", v, "s", s,".RData"))
...
source(file.path(dir.check, "Results_ChangeVar.R"))
