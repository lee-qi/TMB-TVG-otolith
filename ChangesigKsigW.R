...
sigKvec <- seq(from = 0.01, to = 0.1, by = 0.03)
sigWvec <- seq(from = 0.01, to = 0.1, by = 0.03)
...
loopres <- foreach(k = 1:NKe) %:%
			...
			set.seed(8888*as.numeric(paste(s,k,w, sep = "")))
			...
			if(sum(which(optgrad>=10e-6))==0) {
				...
				save(replist, file = file.path(getwd(), dir.new, paste0("k", k, "w", w, "s", s,".RData")))
				return(replist)
			}
		}
#	}
...
loopres <- foreach(k = 1:NKe) %:%
		foreach(w = 1:NWe) %:%
			foreach(s = 1:Nsims) %dopar% {
				repfile <- file.path(dir.check, dir.new, paste0("k", k, "w", w, "s", s,".RData"))
...
source(file.path(dir.check, "Results_ChangeSigKSigW.R"))
